...
def train_predict(model_num):
        model = Sequential()
        if(model_num==1):
                model.add(LSTM(8, batch_input_shape=(testX.shape[0],testX.shape[1], testX.shape[2]),kernel_initializer='glorot_uniform', recurrent_initializer='orthogonal'))
        if(model_num==2):
                model.add(LSTM(8, batch_input_shape=(testX.shape[0],testX.shape[1], testX.shape[2]),kernel_initializer='glorot_normal', recurrent_initializer='orthogonal'))
        if(model_num==3):
                model.add(LSTM(8, batch_input_shape=(testX.shape[0],testX.shape[1], testX.shape[2]),kernel_initializer='glorot_uniform', recurrent_initializer='glorot_normal'))
        if(model_num==4):
                model.add(LSTM(8, batch_input_shape=(testX.shape[0],testX.shape[1], testX.shape[2]),kernel_initializer='he_normal', recurrent_initializer='orthogonal'))
        if(model_num==5):
                model.add(LSTM(8, batch_input_shape=(testX.shape[0],testX.shape[1], testX.shape[2]),kernel_initializer='glorot_uniform', recurrent_initializer='he_normal'))
        if(model_num==6):
                model.add(LSTM(8, batch_input_shape=(testX.shape[0],testX.shape[1], testX.shape[2]),kernel_initializer='lecun_normal', recurrent_initializer='orthogonal'))
        if(model_num==7):
                model.add(LSTM(8, batch_input_shape=(testX.shape[0],testX.shape[1], testX.shape[2]),kernel_initializer='glorot_uniform', recurrent_initializer='lecun_normal'))
        if(model_num==8):
                model.add(LSTM(8, batch_input_shape=(testX.shape[0],testX.shape[1], testX.shape[2]),kernel_initializer='RandomNormal', recurrent_initializer='orthogonal'))
        if(model_num==9):
                model.add(LSTM(8, batch_input_shape=(testX.shape[0],testX.shape[1], testX.shape[2]),kernel_initializer='glorot_uniform', recurrent_initializer='RandomNormal'))
        ...
        model.fit(trainX_full, trainY_full, validation_data=(validationX, validationY),epochs=50, batch_size=testX.shape[0], verbose=2, shuffle=False,callbacks=[checkpointer, earlystopper,reduce_lr])



        validationPredict=model.predict(validationX,batch_size=144)
        validation_mae=mean_absolute_error(validationY, validationPredict)

        testPredict = model.predict(testX,batch_size=144)
...
for i in range(1,9):
        ...
        testPredict_sin,validation_mae_sin=train_predict(i)
        ...
        testPredict_cos,validation_mae_cos=train_predict(i)
